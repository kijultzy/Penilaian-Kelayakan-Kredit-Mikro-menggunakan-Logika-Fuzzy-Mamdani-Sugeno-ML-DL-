...
\begin{enumerate}[label=\arabic*.]
  ...
  \item Basis aturan dibatasi 22 aturan \textit{IF-THEN}.
\end{enumerate}
...
Sistem menggunakan 22 aturan \textit{IF-THEN} dengan operator
\texttt{AND} (nilai minimum). Tabel~\ref{tab:rule_base}
merangkum seluruh aturan.

\begin{table}[H]
  \centering
  \caption{Matriks 22 Basis Aturan Fuzzy}
  \label{tab:rule_base}
...
\subsection{Inferensi 22 Aturan}

\begin{lstlisting}[caption={Inferensi 22 aturan dengan operator AND (minimum)}]
aturan = [
    # --- BAIK ---
...
% [INSTRUKSI] Ambil screenshot Streamlit dengan Skenario B, lalu:
% 1. Simpan sebagai screenshot_app.png di folder yang sama
% 2. Ganti baris fbox di bawah dengan:
%    \includegraphics[width=0.9\textwidth]{screenshot_app.png}

\begin{figure}[H]
  \centering
  \fbox{\parbox{0.85\textwidth}{%
    \centering\vspace{1.5cm}
    {\itshape Lampirkan tangkapan layar antarmuka Streamlit di
    sini.\\[4pt]
    Jalankan: \texttt{python -m streamlit run app.py}\\
    Atur input ke Skenario B, lalu ambil screenshot halaman utama.}
    \vspace{1.5cm}}}
...
Evaluasi dilakukan terhadap 10.000 sampel acak dari dataset yang
telah dibersihkan (61.340 baris valid dari 100.000 baris mentah).
Penggunaan 10.000 sampel memastikan representasi statistik yang
kuat dan memenuhi ketentuan minimal 5.000 baris data pengujian.
...
\begin{table}[H]
  \centering
  \caption{Perbandingan Performa Mamdani vs.\ Sugeno (n = 10.000 sampel)}
  \label{tab:performa}
  ...
  \begin{tabular}{|l|c|c|c|}
    ...
    \textbf{Lebih Baik} \\
    \hline
    Akurasi Klasifikasi       & 55,45\%  & 55,34\%  & Mamdani \\
    MAE (poin)                & 16,95    & 17,05    & Mamdani \\
    MSE (poin$^2$)            & 663,59   & 671,80   & Mamdani \\
    RMSE (poin)               & 25,76    & 25,92    & Mamdani \\
...
\begin{figure}[H]
  ...
  \caption{Evaluasi Performa Fuzzy --- \textit{Confusion Matrix}
           Mamdani vs.\ Sugeno (n = 10.000 sampel)}
  \label{fig:evaluasi}
\end{figure}
...
\begin{enumerate}[label=\arabic*.]
  \item \textbf{Keterbatasan Aturan.}\ Sistem menggunakan 22
...
\begin{enumerate}[label=\arabic*.]
  \item Sistem logika fuzzy berhasil diimplementasikan
        \textit{from scratch} menggunakan Python dan NumPy
        dengan 22 aturan \textit{IF-THEN}. Kedua algoritma
        (Mamdani-Centroid dan Sugeno-Weighted Average)
        menghasilkan keputusan kelayakan kredit yang
        konsisten.

  \item Evaluasi \textit{batch} terhadap 10.000 sampel dari
        61.340 baris data yang telah dibersihkan menunjukkan
        akurasi \textbf{Mamdani 55,45\%} dan \textbf{Sugeno
        55,34\%} terhadap \textit{ground truth}, dengan MAE
        masing-masing 16,95 dan 17,05 poin, serta RMSE
        masing-masing 25,76 dan 25,92 poin.

  \item Sugeno terbukti jauh lebih efisien ($<$0{,}1 ms per
        inferensi) dibandingkan Mamdani (10--50 ms). Namun
        output Mamdani lebih kontinu dan menghasilkan akurasi
        yang sedikit lebih tinggi. Dalam hal akurasi akhir,
        selisih antara keduanya hanya 0{,}11\%.
